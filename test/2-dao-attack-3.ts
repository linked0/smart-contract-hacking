...
import { AttackDAO, DAOToken, Governance, LendingPool, Treasury } from '../typechain-types';
...
describe('DAO Attack Exercise 3', function () {
	...
	it('Exploit', async function () {
        attackDAO = await deployContract("AttackDAO", [
            token.target, treasury.target, governance.target, pool.target],attacker
        );

        await attackDAO.attack();
	});

	after(async function () {
		/** SUCCESS CONDITIONS */

		console.log('Attacker ETH Balance:', await provider.getBalance(attacker.address));
